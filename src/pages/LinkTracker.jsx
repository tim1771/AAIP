...
export default function LinkTracker() {
  ...
  const [form, setForm] = useState({ url: '', name: '', product: '', source: '', medium: '', campaign: '' })
  ...
  const loadData = async () => {
    ...
    try {
      const [linksRes, productsRes] = await Promise.all([
        supabase.from('affiliate_links').select('*, affiliate_products(product_name), campaigns(name)').eq('user_id', user.id).order('created_at', { ascending: false }),
  ...
  const createLink = async () => {
    ...
    try {
      ...
      const trackedUrl = `${form.url}${form.url.includes('?') ? '&' : '?'}utm_source=${form.source || 'affiliateai'}&utm_medium=${form.medium || 'link'}&utm_campaign=${form.campaign || 'default'}`
      
      await supabase.from('affiliate_links').insert({
        user_id: user.id, original_url: form.url, tracked_url: trackedUrl, short_code: shortCode,
        custom_name: form.name || null, product_id: form.product || null,
        utm_source: form.source || 'affiliateai', utm_medium: form.medium || 'link', utm_campaign: form.campaign || 'default'
      })
      addToast('Link created!', 'success')
      setForm({ url: '', name: '', product: '', source: '', medium: '', campaign: '' })
      loadData()
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="card-body">
          <div className="form-row">
            ...
            <div className="form-group"><label>Link Name</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g., Blog Footer CTA" /></div>
          </div>
          <div className="form-row">
          ...
          {links.length > 0 ? (
            <div className="table-container">
              <table className="data-table">
                <thead><tr><th>Name</th><th>Product</th><th>Source</th><th>Clicks</th><th>Actions</th></tr></thead>
                <tbody>
                  {links.map(link => (
                    <tr key={link.id}>
                      <td>
                        <div style={{ fontWeight: 500 }}>{link.custom_name || 'Unnamed'}</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{link.tracked_url}</div>
                      </td>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <button className="btn btn-sm btn-ghost" onClick={() => handleCopy(link.tracked_url)}>📋</button>
